...
internal sealed unsafe class RustHandle<T> : SafeHandle where T : unmanaged
{
    ...
    internal void ValidateCurrentVersion(MutationVersion mutationVersion)
    {
    ...
    internal BorrowLease<T> LeaseCurrentVersionForAccess(MutationVersion mutationVersion)
    {
        if (!IsCurrent(mutationVersion))
        {
            throw new InvalidOperationException(
